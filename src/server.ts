...
<div class="container">
  ...
  <form id="form">
    ...
    <button type="submit" id="submitBtn">Cast</button>
  </form>
  <div id="result"></div>
</div>
<script>
document.getElementById('form').addEventListener('submit', async (e) => {
  ...
  try {
    ...
    html += \`<div class="hex-title">\${data.primary.character} \${data.primary.chineseName} — \${data.primary.name}</div>\`;
    if (data.resulting) {
      html += '<div class="hex-resulting">';
      html += \`<div class="hex-number">Resulting · Hexagram \${data.resulting.number}</div>\`;
      html += \`<div class="hex-title">\${data.resulting.character} \${data.resulting.chineseName} — \${data.resulting.name}</div>\`;
      html += '</div>';
    }
    html += '</div>';
