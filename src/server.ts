...
<title>Yi Jing</title>
<link rel="icon" href="data:image/svg+xml,<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 100 100'><text y='50%' font-size='75' x='50%' dominant-baseline='central' text-anchor='middle' fill='%237060a8'>易</text></svg>">
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .disclaimer { margin-top: 64px; font-size: 0.78rem; line-height: 1.6; color: #b0a090; font-style: italic; }
  .tabs { margin-top: 24px; }
  .tab-nav { display: flex; gap: 0; border-bottom: 1px solid #e0d4cb; }
  .tab-btn { background: none; border: none; border-bottom: 2px solid transparent; margin-bottom: -1px; padding: 8px 18px 8px 0; font-family: Georgia, serif; font-size: 0.75rem; letter-spacing: 0.08em; text-transform: uppercase; color: #a08878; cursor: pointer; transition: color 0.15s; display: flex; align-items: center; gap: 5px; }
  .tab-btn:hover { background: none; color: #9080c0; }
  .tab-btn.active { background: none; color: #7060a8; border-bottom-color: #c4b8e0; }
  .tab-arrow { display: inline-block; transition: transform 0.2s; font-style: normal; }
  .tab-btn.active .tab-arrow { transform: rotate(90deg); }
  .tab-panel { display: none; padding-top: 20px; }
  .tab-panel.active { display: block; }
  .tab-panel p { font-size: 0.88rem; line-height: 1.75; color: #6a5e55; }
  .tab-panel p + p { margin-top: 10px; }
  .tab-panel a { color: #7060a8; text-decoration: none; border-bottom: 1px solid #c4b8e0; }
  .tab-panel a:hover { border-bottom-color: #7060a8; }
  @media (max-width: 480px) {
    body { padding: 16px 8px; }
    .container { padding: 20px 18px 28px; }
    .hex-row { grid-template-columns: 1fr; row-gap: 16px; }
    .hex-row > label:nth-child(2) { margin-top: 0; }
    .lines-group { gap: 12px; }
...
<div class="container">
  ...
  <form id="form">
    ...
    <button type="submit" id="submitBtn" style="margin-top: 8px;">Interpret</button>
  </form>
  <div id="result"></div>
  <p class="disclaimer">The AI does not invent — it reads. But it can be wrong. Bring your own discernment.</br>Your questions and interpretations are your own — nothing is stored or logged.</p>
  <div class="tabs">
    <div class="tab-nav">
      <button class="tab-btn" data-tab="yijing"><i class="tab-arrow">›</i>the Yi Jing</button>
      <button class="tab-btn" data-tab="interpreter"><i class="tab-arrow">›</i>this Interpreter</button>
    </div>
    <div class="tab-panel" id="tab-yijing">
      <p>The Yi Jing (I Ching) is one of the oldest texts in the world — a Chinese classic of divination and philosophy, composed over centuries, that maps sixty-four hexagrams onto the whole range of human situations. Each hexagram is built from two trigrams (natural images such as water, fire, mountain, and wind) and carries a judgment, an image, and readings for each of its six lines.</p>
      <p>To consult the Yi Jing you first need to cast a hexagram — traditionally with yarrow stalks or three coins. <a href="https://www.onlineclarity.co.uk/cast/" target="_blank" rel="noopener">Online Clarity</a> offers an online casting tool and is a deep well of information on the Yi Jing, its hexagrams, and the practice of casting.</p>
    </div>
    <div class="tab-panel" id="tab-interpreter">
